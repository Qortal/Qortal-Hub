...
import (
	...
	"github.com/quic-go/quic-go/internal/datagrambuffer"
)
...
func newStateTrackingStream(s *quic.Stream, clearer streamClearer, sendDatagram func([]byte) error) *stateTrackingStream {
	t := &stateTrackingStream{
		...
		queue:        datagrambuffer.New(streamDatagramQueueLen),
	}
...
start:
	s.mx.Lock()
	if data, ok := s.queue.Pop(time.Now()); ok {
		s.mx.Unlock()
		return data, nil
	}
...
func (s *stateTrackingStream) enableDatagramReceiveBuffer() {
	s.mx.Lock()
	defer s.mx.Unlock()
	s.queue.EnableBurst()
}
func (s *stateTrackingStream) datagramReceiveBufferStats() quic.DatagramReceiveBufferStats {
	s.mx.Lock()
	defer s.mx.Unlock()
