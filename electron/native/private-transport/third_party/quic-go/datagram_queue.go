...
func newDatagramQueue(hasData func(), logger utils.Logger) *datagramQueue {
	return &datagramQueue{
		...
		logger:   logger,
		rcvQueue: datagrambuffer.New(maxDatagramRcvQueueLen),
	}
}
...
func (h *datagramQueue) HandleDatagramFrame(f *wire.DatagramFrame) {
	var queued bool
	h.rcvMx.Lock()
	if h.rcvQueue.Push(f.Data, time.Now()) {
		queued = true
		select {
		case h.rcvd <- struct{}{}:
		default:
...
func (h *datagramQueue) Receive(ctx context.Context) ([]byte, error) {
	for {
		h.rcvMx.Lock()
		if data, ok := h.rcvQueue.Pop(time.Now()); ok {
			h.rcvMx.Unlock()
			return data, nil
		}
		h.rcvMx.Unlock()
